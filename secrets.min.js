...
function l(a){throw a;}var m=!0,p=null,q=!1,aa=[p,p,1,3,3,5,3,3,29,17,9,5,83,27,43,3,45,9,39,39,9,5,3,33,27,9,71,39,9,5,83],r="WARNING:\nA secure random number generator was not found.\nUsing Math.random(), which is NOT cryptographically strong!",s,t,w,x,y,z,B,C,D;
exports.init=function(a,b,c){a&&("number"!==typeof a||0!==a%1||3>a||20<a)&&l(Error("Number of bits must be an integer between 3 and 20, inclusive."));b&&("number"!==typeof b||0!==b%1||2>b||36<b)&&l(Error("Radix must be an integer between 2 and 36, inclusive."));c&&("number"!==typeof c||0!==c%1||2>c)&&l(Error("Zero pad length must be an integer greater than 1."));s=a||8;t=b||16;w=c||256;x=Math.pow(2,s);y=x-1;if(!z||!(z.length===x&&B&&B.length)){a=[];b=[];c=1;for(var e=aa[s],d=0;d<x;d++)b[d]=c,a[c]=
d,c<<=1,c>=x&&(c^=e,c&=y);z=a;B=b}};function E(){return!s||!t||!w||!z||!B||!x||!y||z.length!==x||B.length!==x?q:m}exports.getConfig=function(){return{bits:s,radix:t,padLength:w}};
...
function F(a,b){b=b||s;var c=a.length%b;return(c?Array(b-c+1).join("0"):"")+a}exports._getShares=function(a,b,c){var e=[];a=[a];for(var d=1;d<c;d++)a[d]=parseInt(D(s),2);d=1;for(b+=1;d<b;d++){c=z[d];for(var g=0,f=a.length-1;0<=f;f--)g=0===g?a[f]:B[(c+z[g])%y]^a[f];e[d-1]={x:d,y:g}}return e};
exports.newShare=function(a,b,c){E()||this.init();c=c||t;("number"!==typeof c||0!==c%1||2>c||36<c)&&l(Error("Radix must be an integer between 2 and 36, inclusive."));"string"===typeof a&&(a=parseInt(a,c));("number"!==typeof a||0!==a%1||1>a||a>y)&&l(Error("Share id must be an integer between 1 and "+y+", inclusive."));return a.toString(c)+"-"+J(a,b,c,c)};
function J(a,b,c,e){for(var d=[],g=[],f=[],j="",h,k=0,n=b.length;k<n;k++){"string"===typeof b[k]?(d=b[k].split("-"),2!==d.length&&l(Error("Invalid share:\n"+b[k]))):"object"===typeof b[k]?(d[0]=b[k].id.toString(),d[1]=b[k].share.toString()):l(Error("Unknown share type:\n"+b[k]));d[0]=parseInt(d[0],c);a:{h=0;for(var u=g.length;h<u;h++)if(g[h]===d[0]){h=m;break a}h=q}if(!h){h=g.push(d[0])-1;d[1]=H(d[1],c);for(var u=0,v=d[1].length;u<v;u++)f[u]=f[u]||[],f[u][h]=d[1][u]}}k=0;for(n=f.length;k<n;k++)j=
F(K(a,g,f[k]).toString(2))+j;return(new I(j,2)).toString(e)}exports.combine=function(a,b,c){E()||this.init();b=b||t;c=c||t;("number"!==typeof b||0!==b%1||2>b||36<b)&&l(Error("Input radix must be an integer between 2 and 36, inclusive."));("number"!==typeof c||0!==c%1||2>c||36<c)&&l(Error("Output radix must be an integer between 2 and 36, inclusive."));return J(0,a,b,c)};
function K(a,b,c){var e=0,d,g,f;g=0;for(var j=b.length;g<j;g++)if(c[g]){d=z[c[g]];for(f=0;f<j;f++)if(g!==f){if(a===b[f]){d=-1;break}d=(d+z[a^b[f]]-z[b[g]^b[f]]+y)%y}e=-1===d?e:e^B[d]}return e}exports._lagrange=K;
exports.toHex=function(a,b){"string"!==typeof a&&l(Error("Input must be a character string."));b=b||2;6<b&&l(Error("Maximum bytes per character is 6."));for(var c=2*b,e=Math.pow(16,c)-1,d="",g,f=0,j=a.length;f<j;f++){g=a[f].charCodeAt();isNaN(g)&&l(Error("Invalid character: "+a[f]));if(g>e){var h=Math.ceil(Math.log(g+1)/Math.log(256));l(Error("Invalid character code ("+g+"). Maximum allowable is 256^bytes-1 ("+e+"). To convert this character, use at least "+h+" bytes."))}d+=F(g.toString(16),c)}return d};
exports.toString=function(a,b){"string"!==typeof a&&l(Error("Input must be a hexadecimal string."));b=b||2;6<b&&l(Error("Maximum bytes per character is 6."));var c=2*b,e="";a=F(a,c);for(var d=0,g=a.length;d<g;d+=c)e+=String.fromCharCode(parseInt(a.slice(d,d+c),16));return e};
...
function ca(a,b,c,e){var d=b.abs();if(!(0>=d.a)){var g=a.abs();if(g.a<d.a)c!=p&&W(c,0),e!=p&&a.copyTo(e);else{e==p&&(e=Q());var f=Q(),j=a.b;b=b.b;var h=d[d.a-1],k=1,n;if(0!=(n=h>>>16))h=n,k+=16;if(0!=(n=h>>8))h=n,k+=8;if(0!=(n=h>>4))h=n,k+=4;if(0!=(n=h>>2))h=n,k+=2;0!=h>>1&&(k+=1);h=a.c-k;0<h?(fa(d,h,f),fa(g,h,e)):(d.copyTo(f),g.copyTo(e));d=f.a;g=f[d-1];if(0!=g){n=g*(1<<a.g)+(1<d?f[d-2]>>a.h:0);k=a.i/n;n=(1<<a.g)/n;var u=1<<a.h,v=e.a,M=v-d,A=c==p?Q():c;V(f,M,A);0<=ga(e,A)&&(e[e.a++]=1,R(e,A,e));
V(ha,d,A);for(R(A,f,f);f.a<d;)f[f.a++]=0;for(;0<=--M;){var U=e[--v]==g?a.d:Math.floor(e[v]*k+(e[v-1]+u)*n);if((e[v]+=f.e(0,U,e,M,0,d))<U){V(f,M,A);for(R(e,A,e);e[v]<--U;)R(e,A,e)}}if(c!=p){a=e;for(f=d;f<a.a;++f)c[f-d]=a[f];c.a=Math.max(a.a-d,0);c.b=a.b;j!=b&&R(S,c,c)}e.a=d;T(e);if(0<h)if(b=c=e,b.b=c.b,a=Math.floor(h/c.c),a>=c.a)b.a=0;else{h%=c.c;f=c.c-h;d=(1<<h)-1;b[0]=c[a]>>h;for(g=a+1;g<c.a;++g)b[g-a-1]|=(c[g]&d)<<f,b[g-a]=c[g]>>h;0<h&&(b[c.a-a-1]|=(c.b&d)<<f);b.a=c.a-a;T(b)}0>j&&R(S,e,e)}}}}
